...
static void lbm_button_handler(uint32_t event)
{
...
{
	button_set_action_short_press(DK_BTN1, lbm_button_handler, SID_EVENT_SEND_MSG);
	// ? TODO what is SID_EVENT_CONNECT ?
	button_set_action(DK_BTN3, lbm_button_handler, SID_EVENT_LINK_SWITCH);
	button_set_action(DK_BTN4, lbm_button_handler, SID_EVENT_TOGGLE_LBM_SIDEWALK);
...
/* sid_init()
 * 1) state_sidewalk_entry()
 * 2) SID_EVENT_LINK_SWITCH in state_sidewalk_run()
 */
void app_start(void)
{
	if (lbm_buttons_init()) {
